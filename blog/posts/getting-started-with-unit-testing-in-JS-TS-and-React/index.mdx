...
<Admonition type="note" title={<LangChanger labels={["JS,", "TS", "and React"]} containerClass={"flex flex-row gap-2"}/>} className={"mt-2"}>

<div>This post is available for <LangChanger labels={["JavaScript,", "TypeScript", "and React"]} containerClass={"inline-flex flex-row gap-2"}/></div>
