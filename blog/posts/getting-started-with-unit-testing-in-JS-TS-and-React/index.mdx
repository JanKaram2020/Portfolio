...
<Admonition type="note" title={<LangChanger labels={["JS,", "TS", "and React"]} buttonClass={"hover:underline"} containerClass={"flex flex-row"} variant={"space"}  className={"mt-2"}/>}>

<div>This post is available for <LangChanger labels={["JavaScript,", "TypeScript", "and React"]} buttonClass={"hover:underline"} containerClass={"inline-flex flex-row"} variant={"space"} /> </div>
