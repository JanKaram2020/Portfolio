...
const AnimatedLogo = forwardRef((props, ref) => {
  const { scrollY } = useScroll();
  const dimensions = useLogoPosition();
  const width = useTransform(scrollY, [0, 800], ["80%", dimensions.width]);
  const top = useTransform(scrollY, [0, 800], [400, 20]);
  const left = useTransform(scrollY, [0, 800], ["10%", dimensions.left]);
