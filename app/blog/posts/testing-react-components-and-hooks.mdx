export const frontmatter = {
  title: "Testing React hooks and Components",
  summary:
...
missed it [here](http://localhost:3000/blog/2023/10/25/getting-started-with-unit-testing-in-js-ts-and-react) Now that we
...
While we initially included a separate test to verify the button rendered correctly, it's not necessary.<br/> Testing
the `onClick` behavior inherently checks for rendering as well. When a button is clicked, it must be rendered properly in the
document.<br/> we can now remove the first test and be sure that both cases are covered by the `onClick` test.<br/> This
allows us to streamline our tests and focus on the core functionalities.<br/>
in short: if you check any functionality other than render you test the render implicitly
...
We want to ensure that the hook works correctly and that count value is correct and increments and decrements when
...
- when doing an action that changes the state we must use the `act` method (it tells the testing library something about to change in the render so update it after the call)<br/>
...
- it should remove the two event listeners when unmounted (to avoid leaky subscription)
