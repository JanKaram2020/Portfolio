...
import { MultiLangCodeBlock, MultiLangCodeRoot } from "../components/MultiLangCodeBlock";

<MultiLangCodeRoot values={["js","ts","react"]}>
...
<br/>
this post covers testing javascript, typescript and react's enviroment setup
you can choose at every block what you want to setup
...
``` shell
yarn add -D vitest vite jsdom shell
```

```shell
yarn add -D vitest vite jsdom shell js
```


```shell
yarn add -D vitest vite jsdom shell ts
```
...
### Make a file in the root of the project called `vitest.config.ts{:plaintext}` with the following code
